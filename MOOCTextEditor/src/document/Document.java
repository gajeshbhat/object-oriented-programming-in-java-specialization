...
public abstract class Document {
	...
	protected int countSyllables(String word) {
		...
		// Change word to lower case to simplify vowel check.
		word = word.toLowerCase();
		for (int i = 0; i < wordLen; i++) {
			// Let's store the current character as a CharSequence so we can use the
			// String.contains(CharSequence) method to check if the current character is a vowel.
			CharSequence currentChar = word.substring(i, i + 1);
			// 'y' is considered a vowel in the context of a syllable.
			boolean isVowel = "aeiouy".contains(currentChar);
			if (isVowel) {
				if (i == wordLen - 1) {
					if (!isVowelSeq && numOfSyllables != 0 && currentChar.equals("e"))
